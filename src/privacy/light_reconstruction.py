...
DEFAULT_METHODS: tuple[str, ...] = ("pinv", "mlp", "pinv_not_orth")
...
def run_light_anchor_reconstruction(
    ...
    *,
    ...
    method_keys = tuple(str(m).lower() for m in methods)
    for m in method_keys:
        if m not in {"pinv", "mlp", "pinv_not_orth"}:
            raise ValueError(f"Unsupported method '{m}'. Supported: pinv, mlp, pinv_not_orth.")
    ...
    for method in method_keys:
        if method == "pinv":
            ...
            if reconstruct_anchor_images:
                anchor_recons[method] = _reconstruct_pinv_centered(
            ...
            if reconstruct_anchor_images:
                anchor_recons[method] = _reconstruct_mlp(
                    anchor_tilde,
                    anchor_X,
                    anchor_tilde,
                    seed=seed,
            ...
            if reconstruct_anchor_images:
                anchor_recons[method] = _reconstruct_pinv_not_orth(
                    anchor_tilde, anchor_X, anchor_tilde, center=recon_centering
                )
    ...
    if evaluate_classifier_accuracy:
        ...
        for method in method_keys:
            ...
            metrics[method] = {"classifier_accuracy": float(np.mean(y_pred == y_true_selected))}
