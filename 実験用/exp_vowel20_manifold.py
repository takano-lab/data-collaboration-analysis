...
class ExpCfg:
    ...
    n_samples: int = 20
    n_components: int = 3
    out_dir: Path = Path("output") / "vowel20_manifold"
...
def _label_encode(y: np.ndarray) -> np.ndarray:
    if y.dtype.kind in {"i", "u"}:  # ints already
        return y.astype(int)
    le = LabelEncoder()
    return le.fit_transform(y)


def load_vowel() -> Tuple[np.ndarray, np.ndarray]:
    """Load vowel dataset robustly from OpenML, keeping only numeric features."""
    try:
    ...
    if Xdf_num.shape[1] == 0:
        raise RuntimeError("vowel dataset has no numeric columns after filtering")
    X = Xdf_num.to_numpy(dtype=np.float32)
    y = _label_encode(np.asarray(d["target"]))
    return X, y


def sample_balanced_20(X: np.ndarray, y: np.ndarray, n: int, seed: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Select near-balanced n samples across classes:
    - Ensure min 1 per class (if n >= n_classes)
    - Distribute the remaining uniformly over classes (round-robin)
    Returns (Xn, yn, idx)
    """
    ...
    k = len(classes)
    if n < k:
        # Not enough to cover all classes: pick n distinct classes at random, 1 each
        chosen_classes = rng.choice(classes, size=n, replace=False)
        idx_list = []
        for c in chosen_classes:
            cand = np.where(y == c)[0]
            idx_list.append(rng.choice(cand))
        idx = np.array(idx_list, dtype=int)
        return X[idx], y[idx], idx

    # Start with 1 per class
    taken_per_cls = {int(c): 1 for c in classes}
    remain = n - k
    # Compute pools per class
    pools = {int(c): np.where(y == c)[0] for c in classes}
    # For the first one per class, sample 1 each
    chosen: List[int] = []
    for c in classes:
        idx_c = pools[int(c)]
        chosen.append(int(rng.choice(idx_c)))
    ...
    # Distribute remaining in round-robin over classes with available samples
...
def _median_heuristic_gamma(X: np.ndarray) -> float:
    # 1/(2*med^2) on pairwise euclidean distances
...
def embed_diffusion_maps(X: np.ndarray, k: int, *, gamma: Optional[float] = None) -> np.ndarray:
    """
    Simple diffusion maps (t=1):
    - K_ij = exp(-gamma * ||xi-xj||^2)
    - P = D^{-1} K (row-stochastic)
    - Take eigenvectors of P, use top 3 non-trivial components (skip first trivial eigenvector)
    """
    if gamma is None:
    ...
    P = K / row_sum
    # Eigen decomposition (right eigenvectors of P^T equivalently)
...
def plot_3d(Z: np.ndarray, y: np.ndarray, title: str, out_path: Path) -> None:
    ...
    sc = ax.scatter(Z[:, 0], Z[:, 1], Z[:, 2], c=y, cmap="tab20", s=40, alpha=0.9)
    ax.set_title(title)
    ax.set_xlabel("z1")
    ax.set_ylabel("z2")
    ax.set_zlabel("z3")
    # make a simple legend (unique labels)
...
# ----------------------
def main():
    ap = argparse.ArgumentParser(description="Vowel 20-sample manifold experiment (3D + Linear SVC)")
    ...
    # 1) Load and sample 20 near-balanced points
    X, y = load_vowel()
    X20, y20, idx = sample_balanced_20(X, y, n=cfg.n_samples, seed=cfg.seed)

    # Standardize once for DR
    ...
    for nn in cfg.umap_neighbors:
        for md in cfg.umap_min_dist:
            try:
                ...
                acc, p = eval_linear_svc_cv(Z, y20, cfg.lin_svm_C, cfg.seed)
                plot_3d(Z, y20, f"UMAP nn={nn}, md={md} (acc={acc:.3f})", cfg.out_dir / f"vowel20_umap_nn{nn}_md{md}_3d.png")
    ...
    # 6) Diffusion Maps (gamma via median heuristic and 2x)
    try:
    ...
    for g in (g_med, 2 * g_med):
        try:
            ...
            acc, p = eval_linear_svc_cv(Z, y20, cfg.lin_svm_C, cfg.seed)
            plot_3d(Z, y20, f"DiffusionMaps g={g:.2e} (acc={acc:.3f})", cfg.out_dir / f"vowel20_diffmap_g{g:.2e}_3d.png")
    ...
    # Save results and sampling info
    res_df = pd.DataFrame(results)
    res_path = cfg.out_dir / "vowel20_results.csv"
    ...
    pd.DataFrame([info]).to_json(cfg.out_dir / "vowel20_meta.json", orient="records", force_ascii=False, indent=2)

    # Print the best by acc
